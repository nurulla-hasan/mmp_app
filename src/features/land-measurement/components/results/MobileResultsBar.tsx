...
const buildReportHtml = (state: ReturnType<typeof useMapStore.getState>, imageData: string | null) => {
  ...
  const overlay = plots.map((plot) => {
    ...
    const edges = groupPolygonSegments(plot.points).map((group, groupIndex) => {
    ...
    const area = `<text x="${center.x}" y="${center.y}" class="area" fill="${color}">${escapeHtml(`${plot.results.shotok.toFixed(2)} শতক`)}</text>`;

      ...
      <View style={[styles.backdrop, { backgroundColor: colors.overlayStrong }]}>
        ...
        <View style={[styles.sheet, { backgroundColor: colors.panel, borderColor: colors.panelBorder }]}>
          ...
          <View style={styles.header}>
            <View>
              <Text style={[styles.title, { color: colors.textStrong }]}>Choose Mouza Map</Text>
              <Text style={[styles.subtitle, { color: colors.textSoft }]}>Use PDF, JPG, PNG, or a photo from your phone</Text>
            </View>
            <TouchableOpacity disabled={isImportingPdf} style={[styles.closeButton, { backgroundColor: colors.panelRaised }]} onPress={onClose}>
          ...
          {mapImage && (
            <View style={styles.currentFile}>
              <FileImage size={18} color={colors.success} />
              <View style={styles.currentFileText}>
                <Text numberOfLines={1} style={[styles.currentName, { color: colors.textStrong }]}>{mapImage.name ?? 'Current map'}</Text>
                <Text style={[styles.currentMeta, { color: colors.textSoft }]}>{mapImage.width} × {mapImage.height} px</Text>
              </View>
            </View>
          )}

          <TouchableOpacity disabled={isImportingPdf} activeOpacity={0.76} style={[styles.option, { backgroundColor: colors.panelAlt, borderColor: colors.panelBorder }, isImportingPdf && styles.disabledOption]} onPress={pickImage}>
            <View style={[styles.optionIcon, styles.greenIcon]}>
              <ImagePlus size={22} color={colors.success} />
            </View>
            <View style={styles.optionText}>
              <Text style={[styles.optionTitle, { color: colors.textStrong }]}>Choose from Gallery</Text>
              <Text style={[styles.optionDescription, { color: colors.textSoft }]}>Select a mouza map image saved on your phone</Text>
            </View>
          </TouchableOpacity>

          <TouchableOpacity disabled={isImportingPdf} activeOpacity={0.76} style={[styles.option, { backgroundColor: colors.panelAlt, borderColor: colors.panelBorder }, isImportingPdf && styles.disabledOption]} onPress={pickPdf}>
            <View style={[styles.optionIcon, styles.pdfIcon]}>
              {isImportingPdf ? <ActivityIndicator color='#ef4444' /> : <FileText size={22} color='#ef4444' />}
            </View>
            <View style={styles.optionText}>
              <Text style={[styles.optionTitle, { color: colors.textStrong }]}>{isImportingPdf ? 'Preparing PDF…' : 'Import PDF Map'}</Text>
              <Text style={[styles.optionDescription, { color: colors.textSoft }]}>{isImportingPdf ? 'Converting page 1 to a high-resolution image' : 'Open the first PDF page on the measurement canvas'}</Text>
            </View>
          </TouchableOpacity>

          {mapImage && <TouchableOpacity disabled={isImportingPdf} activeOpacity={0.76} style={[styles.option, { backgroundColor: colors.panelAlt, borderColor: colors.panelBorder }, isImportingPdf && styles.disabledOption]} onPress={removeMap}>
            <View style={[styles.optionIcon, styles.redIcon]}><Trash2 size={21} color='#ef4444' /></View>
            <View style={styles.optionText}><Text style={[styles.optionTitle, { color: colors.textStrong }]}>Remove Current Map</Text><Text style={[styles.optionDescription, { color: colors.textSoft }]}>Clear the map and all current measurements</Text></View>
          </TouchableOpacity>}
        </View>
      </View>
    </Modal>
...
const styles = StyleSheet.create({
  ...
  closeButton: { width: 34, height: 34, alignItems: 'center', justifyContent: 'center', borderRadius: 9 },
  currentFile: { marginBottom: 10, flexDirection: 'row', alignItems: 'center', gap: 10, padding: 11, borderRadius: 11, borderWidth: 1, borderColor: 'rgba(34, 197, 94, 0.25)', backgroundColor: 'rgba(34, 197, 94, 0.08)' },
  currentFileText: { flex: 1 },
  currentName: { fontFamily: Fonts.headingSemiBold, fontSize: 12 },
  currentMeta: { fontSize: 9.5 },
  option: { marginTop: 9, flexDirection: 'row', alignItems: 'center', gap: 12, padding: 13, borderRadius: 13, borderWidth: 1 },
  ...
  optionDescription: { marginTop: -1, fontFamily: Fonts.sansRegular, fontSize: 10 },
});

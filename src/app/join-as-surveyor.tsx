...
import { BadgeCheck, BriefcaseBusiness, Clock3, ShieldAlert } from 'lucide-react-native';
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: colors.background }]} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps='handled'>
      <View style={styles.header}>
        ...
        <Text style={[styles.subtitle, { color: colors.textMuted }]}>আপনার পেশাদার প্রোফাইল তৈরি করুন এবং নতুন ক্লায়েন্টদের সাথে যুক্ত হন।</Text>
      </View>

      {!isAuthenticated ? (
        <View style={[styles.stateCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <BriefcaseBusiness size={34} color={colors.primary} />
          <Text style={[styles.stateTitle, { color: colors.text }]}>আবেদন করতে লগইন করুন</Text>
          <Text style={[styles.stateText, { color: colors.textMuted }]}>সার্ভেয়ার আবেদন আপনার অ্যাকাউন্টের সাথে যুক্ত হবে। আগে সাইন ইন করুন।</Text>
          <Button title='লগইন করুন' onPress={() => router.push('/(auth)/login')} style={{ minWidth: 130 }} />
        </View>
      ) : profile ? (
        <View style={[styles.stateCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
          {profile.verificationStatus === 'APPROVED' ? <BadgeCheck size={38} color={colors.primary} /> : profile.verificationStatus === 'REJECTED' ? <ShieldAlert size={38} color='#ef4444' /> : <Clock3 size={38} color='#d97706' />}
          <Text style={[styles.stateTitle, { color: colors.text }]}>আপনার সার্ভেয়ার আবেদন</Text>
          <Badge
            label={profile.verificationStatus === 'APPROVED' ? 'অনুমোদিত' : profile.verificationStatus === 'REJECTED' ? 'প্রত্যাখ্যাত' : 'যাচাইয়ের অপেক্ষায়'}
            variant={profile.verificationStatus === 'APPROVED' ? 'success' : profile.verificationStatus === 'REJECTED' ? 'warning' : 'warning'}
          />
          <Text style={[styles.stateText, { color: colors.textMuted }]}>
            {profile.verificationStatus === 'APPROVED'
              ? 'আপনার প্রোফাইল এখন পাবলিক সার্ভেয়ার ডিরেক্টরিতে সক্রিয়।'
              : profile.verificationStatus === 'REJECTED'
                ? (profile.adminNote || 'আবেদনটি অনুমোদন করা হয়নি। বিস্তারিত জানতে সাপোর্টের সাথে যোগাযোগ করুন।')
                : 'অ্যাডমিন আপনার পেশাদার তথ্য যাচাই করছে। অনুমোদনের পর প্রোফাইল ডিরেক্টরিতে দেখা যাবে।'}
          </Text>
          {profile.verificationStatus === 'APPROVED' && user?.role === 'SURVEYOR' ? (
            <View style={styles.actions}>
              <Button title='প্রোফাইল এডিট করুন' onPress={() => router.push('/surveyor-profile')} />
              {profile.slug ? <Button title='পাবলিক প্রোফাইল' variant='outline' onPress={() => router.push({ pathname: '/surveyors/[slug]', params: { slug: profile.slug! } })} /> : null}
            </View>
          ) : null}
        </View>
...
const styles = StyleSheet.create({
  ...
  stateCard: { borderWidth: 1, borderRadius: 15, padding: 26, alignItems: 'center', gap: 9 },
  ...
  actions: { flexDirection: 'row', flexWrap: 'wrap', gap: 8, justifyContent: 'center', marginTop: 4 },

          ...
          <View style={styles.features}>
            {[0, 1, 2, 3].map((feature) => (
              <View key={`feature-skeleton-${item}-${feature}`} style={styles.featureRow}>
                ...
                <LoadingSkeleton
                  opacity={opacity}
                  color={colors.skeletonSoft}
                  style={[styles.skeletonFeature, feature % 2 ? styles.skeletonFeatureShort : null]}
                />
              </View>
            ))}
          </View>
    ...
    >
          ...
          <View style={styles.flexOne}>
            <View style={styles.statusTitleRow}>
              <Text style={[styles.statusTitle, { color: colors.text }]}>
                বর্তমান প্ল্যান: {activeSub.plan?.name || 'Pro Plan'}
              </Text>
      ...
      {pendingSub ? (
        ...
        >
          ...
          <View style={styles.flexOne}>
            <View style={styles.statusTitleRow}>
              <Text style={[styles.statusTitle, { color: colors.text }]}>
                পেমেন্ট যাচাইাধীন: {pendingSub.plan?.name || 'Pro Plan'}
              </Text>
              <Badge label='Pending' variant='warning' />
            </View>
            <Text style={[styles.statusText, { color: colors.textMuted }]}>
              {pendingSub.paymentMethod} • TrxID {pendingSub.transactionId}
            </Text>
          </View>
        </TouchableOpacity>
      ) : null}
      ...
      {plansQuery.isLoading ? (
        ...
        <View style={[styles.errorCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <Text style={[styles.errorTitle, { color: colors.text }]}>প্ল্যান লোড করা যায়নি</Text>
          <Text style={[styles.stateText, { color: colors.textMuted }]}>
            ইন্টারনেট সংযোগ পরীক্ষা করে আবার চেষ্টা করুন।
          </Text>
          <Button title='আবার চেষ্টা করুন' size='sm' onPress={() => plansQuery.refetch()} />
        </View>
      ) : plans.length === 0 ? (
        <View style={[styles.errorCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <Text style={[styles.errorTitle, { color: colors.text }]}>এখন কোনো Pro plan পাওয়া যাচ্ছে না</Text>
          <Text style={[styles.stateText, { color: colors.textMuted }]}>
            Active plan প্রকাশ হলে এখানে দেখা যাবে।
          </Text>
        </View>
      ) : (
        <View style={styles.planList}>
          {plans.map((plan, index) => {
            ...
            const isDowngrade = Boolean(
              activeSub &&
                !isActivePlan &&
                activePlan &&
                plan.durationDays < activePlan.durationDays
            );
            ...
            return (
              ...
              >
                <View style={styles.planTopRow}>
                  ...
                  >
                    <Icon size={20} color={isActivePlan ? '#16a34a' : colors.primary} />
                  </View>
                  <View style={styles.flexOne}>
                    <View style={styles.planNameRow}>
                      <Text style={[styles.planName, { color: colors.text }]}>{plan.name}</Text>
                      {isActivePlan ? (
                        <Badge label='বর্তমান' variant='success' />
                      ) : isPendingPlan ? (
                        <Badge label='Pending' variant='warning' />
                      ) : isRecommended ? (
                        <Badge label={plan.discountBadge || 'সেরা পছন্দ'} variant='pro' />
                      ) : null}
                    </View>
                    {plan.discountBadge && !isRecommended && !isActivePlan ? (
                ...
                {isActivePlan && activeSub?.endDate ? (
                  <Text style={[styles.expiry, { color: '#15803d' }]}>
                    📅 মেয়াদ শেষ: {formatDate(activeSub.endDate)}
                  </Text>
                ) : null}
                ...
                <View style={styles.features}>
                  {(plan.features || []).map((feature) => (
                    <View key={feature} style={styles.featureRow}>
                      <View style={[styles.checkCircle, { backgroundColor: `${colors.primary}12` }]}>
                ...
                {isDowngrade ? (
                  <View style={styles.downgradeBox}>
                    <Button title='ডাউনগ্রেড অনুপলব্ধ' variant='outline' disabled onPress={() => {}} />
                    <Text style={[styles.downgradeText, { color: colors.textMuted }]}>
      ...
      <View style={styles.accessSection}>
        ...
        <View style={[styles.accessCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <View style={styles.accessHeader}>
            ...
            <Text style={[styles.accessTitle, { color: colors.text }]}>Free Access</Text>
            <Badge label='৳০' variant='free' />
          </View>
          {FREE_FEATURES.map((item) => (
            <View key={item} style={styles.accessFeature}>
              <Check size={12} color={colors.textMuted} />
              <Text style={[styles.accessFeatureText, { color: colors.textMuted }]}>{item}</Text>
            </View>
          ))}
        </View>
        <View
        ...
        >
          <View style={styles.accessHeader}>
            ...
            <Text style={[styles.accessTitle, { color: colors.text }]}>Pro Access</Text>
            <Badge label='PRO' variant='pro' />
          </View>
          {PRO_FEATURES.map((item) => (
            <View key={item} style={styles.accessFeature}>
              <Check size={12} color={colors.primary} />
              <Text style={[styles.accessFeatureText, { color: colors.text }]}>{item}</Text>
            </View>
          ))}
        </View>
      </View>
...
const styles = StyleSheet.create({
  ...
  planName: { fontSize: 15.5, fontFamily: Fonts.headingBold },
  ...
  price: { fontSize: 26, fontFamily: Fonts.headingBold },
  ...
  duration: { marginTop: -5, fontSize: 9.5, fontFamily: Fonts.sansRegular },
  expiry: {
    paddingHorizontal: 9,
    paddingVertical: 7,
    borderRadius: 8,
  ...
  features: { gap: 7, marginVertical: 1 },

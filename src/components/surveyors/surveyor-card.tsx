    ...
    >
      <View style={styles.headerRow}>
        {imageUrl ? (
          ...
          <View style={[styles.avatar, styles.avatarFallback, { backgroundColor: `${colors.primary}18` }]}>
            <Text style={[styles.avatarText, { color: colors.primary }]}>{name.charAt(0) || 'স'}</Text>
          </View>
        )}
        ...
        <View style={styles.identity}>
          ...
          {surveyor.headline ? (
            <Text style={[styles.headline, { color: colors.textMuted }]} numberOfLines={1}>{surveyor.headline}</Text>
          ) : null}
        ...
        <View style={styles.ratingBox}>
          {rating > 0 ? (
            <>
              ...
              <Text style={[styles.ratingText, { color: colors.text }]}>{toBengaliDigits(rating.toFixed(1))}</Text>
              {reviews > 0 ? <Text style={[styles.reviewCount, { color: colors.textMuted }]}>({toBengaliDigits(reviews)})</Text> : null}
            </>
          ) : (
      ...
      {!compact && services.length > 0 ? (
        <View style={styles.tagsRow}>
          ...
          {services.length > 3 ? <Badge label={`+${toBengaliDigits(services.length - 3)} আরও`} variant='neutral' /> : null}
        </View>
      ) : null}
